...
\end{description}			
		...
		Anki允许你定制LaTeX 序言所以你可以导入自定义软件包为化学、音乐等。例如，想象你在网上找到一个加入的例子文件：
		...
		使用封装的Anki，点击主窗口的“添加”，然后单击“笔记类型选择按钮。单击“管理”按钮，然后选择要使用的笔记类型，然后单击“选项”。LaTeX的头部和尾部会显示。头部会看起来像什么：
		...
		在编写数学方程式时，在LaTeX码中弹出{{ and }}并不少见。确保你的LaTeX方程与Anki的替代品领域的不冲突，改变分离器的东西是可能的。
		例如，如果你有一个模板：
		...
		虽然在LaTeX中这个经常发生，在你需要包括卡上的{{ }} ，无论是否使用LaTeX的任何情况下这里提出的解决方案将工作。
		...
		将会（和 LaTeX忽略空间的数学模式，所以你的方程将呈现相同的）。
		...
		（请参阅 \url{#deckoptions}记忆库选项 小节中提到的用 \textbf{斜体}标出的选项）。
